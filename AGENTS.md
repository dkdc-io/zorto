...
bin/check-rs       # Rust checks (fmt, clippy, machete, test)
...
Rust checks: `cargo fmt -- --check`, `cargo clippy --all-targets --all-features --workspace -- -D warnings`, `cargo machete`, `cargo test --all-targets --all-features --workspace`
